...
Frontend	Tailwind CSS / CSS	Responsive styling and layout
...
•	User Input: The user types a legal question or complaint description in the React frontend.
•	API Request: The frontend sends an HTTP POST request to the FastAPI backend with the user's query.
•	Embedding: The backend converts the query into a vector embedding using a sentence transformer model.
•	FAISS Retrieval: The query embedding is compared against the FAISS index. The top-K most relevant legal text chunks are retrieved.
•	Prompt Construction: The retrieved legal context is combined with the user's query into a structured prompt.
•	Groq API Call: The prompt is sent to the Groq API which runs the LLM and returns a legal explanation.
•	Response: The backend parses the LLM response, identifies the relevant IPC sections, and returns the structured JSON to the frontend.
•	Display: The frontend renders the legal explanation and IPC sections panel side by side.
...
•	 - A curated dataset of Indian Penal Code sections, IT Act provisions, and legal definitions is preprocessed and chunked into small passages.
•	 - Each passage is converted to a dense vector using a sentence transformer. These vectors are stored in a FAISS index for millisecond-speed similarity search.
•	 - When a user submits a query, it is also converted to a vector. FAISS finds the passages most similar to the query.
•	 - The matched passages are injected into the LLM prompt as context. This grounds the model's response in factual legal text.
•	 - The Groq API LLM generates a coherent, contextually accurate legal explanation using the provided context.
...
•	A Legal Explanation section below that renders the AI's detailed response
